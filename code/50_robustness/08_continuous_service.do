* 98c_continuous_service_routes_csdid2.do
* Exit robustness: restrict treated sample to routes where Frontier served
* continuously (no service gaps) while present on the route.
*
* Complementary to 98b (never-exited routes). This restriction keeps routes
* where F9 maintained consistent competitive pressure, even if it eventually
* exited. 98b keeps routes where treatment was never reversed.
*
* Cross-tab (route-level):
*   continuous + never-exited: 166 routes
*   continuous + exited:       180 routes
*   non-continuous + never-exited: 173 routes
*   non-continuous + exited:    57 routes
*   Total continuous: 346 routes (this sample)
*   Total never-exited: 339 routes (98b sample)
*
* Outcomes:
*   1. Log average fare
*   2. SA log departures
...
local out_root "${results_rob}/frontier_exit/continuous_service"
...
* ==========================================================================
*  LOAD DATA & RESTRICT TO CONTINUOUS-SERVICE TREATED ROUTES
* ==========================================================================

use "`in_dta'", clear

* Drop treated routes where F9 did NOT serve continuously; keep all never-treated
drop if f9_continuous_service == 0 & is_f9_entry_route == 1

di as result "=== Sample after dropping non-continuous treated routes ==="
...
di as result "Treated routes (F9 continuous service): `n_treated'"
...
local stub1 "contsvc_lnfare_maincov_never_dripw"
local stub2 "contsvc_sa_lndep_maincov_never_dripw"
local stub3 "contsvc_carrdelay_maincov_never_dripw"
local stub4 "contsvc_arrival15_maincov_never_dripw"
local stub5 "contsvc_cancel_maincov_never_dripw"
local stub6 "contsvc_sa_lnpax_maincov_never_dripw"
local stub7 "contsvc_sa_lnseats_maincov_never_dripw"
...
postfile `psumm' ///
    ...
    double att se pvalue ll ul using "`out_summary'/contsvc_att_summary.dta", replace
...
forvalues i = 1/`n_outcomes' {
    ...
    di as result "================================================================"
    di as result "  OUTCOME `i'/`n_outcomes': `ylab' (continuous-service routes)"
    ...
    * Adaptive y-axis scaling
    ...
    xlabel(-${wpre_main}(2)${wpost_main}, labsize(medium)) ///
        ...
        title("`ylab' — Continuous-Service Routes", size(medium)) ///
        legend(off) ///
...
postclose `psumm'
use "`out_summary'/contsvc_att_summary.dta", clear
export delimited using "`out_summary'/contsvc_att_summary.csv", replace
...
di as result _newline(3)
di as result "================================================================"
di as result "  CONTINUOUS-SERVICE ROBUSTNESS COMPLETE — 7 OUTCOMES"
di as result "================================================================"
di as result "  Sample: 346 treated routes where F9 served continuously"
di as result "  (dropped 230 routes with service gaps)"
di as result ""
di as result "  Complementary to 98b (never-exited, 339 routes):"
di as result "    - Never-exited: treatment never reversed"
di as result "    - Continuous:   consistent competitive pressure while present"
di as result ""
